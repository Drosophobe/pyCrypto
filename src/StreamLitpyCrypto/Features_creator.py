...
def create_features(snr, mrkt, window, algos, noms_algos):
    ...
    end_date = df.index[-1]
    df_size= df.shape[0]
    df = df.drop(columns = ["^VIX_Vol", "^VIX_Close"], axis=1)
    ...
    #print("nombre de window :", wndw_nbr)
    #print("taille du df :", df_size)
    #print('nombre effective de window :', wndw_nbr*window)
    T = S.copy()[[]]
    ...
    #print(df_len_cryto_tweets.loc[date_list[1]:date_list[2]])
    for i in range(len(date_list)):
        if i == 0:
            ...
            days = 1
        #print(i)
        #print(df_len_cryto_tweets.loc[date_list[i]+timedelta(days=days):date_list[i +1]])
        ...
        if i == len(date_list) - 2:
            break
    #print(my_len_cryptos_tweets_list)
    #print(my_len_nasdaq_tweets_list)
    ...
    my_vol_range_list = []
    #print(T['Mean_Close_Volatility'])
    # Si y'a un pb recheck ça mon bro <3 T'es trop fort Pierre JTM 4ever
    ...
    #print(my_close_range_list)
    #print(my_close_volatility_list)
    #print(my_vol_volatility_list)
    #print(my_vol_range_list)
    df_features = pd.read_csv("../../data/csv_features/Features.csv", index_col=0, parse_dates=True)
    #print(df_features[date_list[0]: date_list[1]])
    #print(df_features[date_list[1]-timedelta(days=1): date_list[2]])
    ...
    for l in range(wndw_nbr):
        if l ==0:
            ...
            days = 1
        #print(df_features[date_list[l]+timedelta(days=days): date_list[l+1]])
    ...
    for m in range(wndw_nbr):
        ...
        print(df_features['Gold_Volume'][date_list[m] + timedelta(days=days): date_list[m + 1]])
        #print(T['Volatility_Gold_Vol'].isna().sum())
...
for i in mrkt_list:
    for j in snr_list:
        create_features(snr=j, mrkt=i, algos= algorithmes, noms_algos= noms_algos, window=7)